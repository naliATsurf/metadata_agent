...
It mirrors ``examples/weave_tracking.py`` step for step, swapping W&B/Weave for
MLflow so the two tracking backends can be compared side by side.
"""
...
# MLflow 3 rejects the ./mlruns file store, so default to a local SQLite backend.
# Point MLFLOW_TRACKING_URI at a tracking server in .env to override it.
mlflow.set_tracking_uri(os.getenv("MLFLOW_TRACKING_URI", "sqlite:///mlflow.db"))
mlflow.set_experiment("metadata-agent-mlflow")
# Weave auto-patches LLM libraries on init; MLflow needs autolog to be explicit.
mlflow.langchain.autolog()

class TrackedOrchestrator(Orchestrator):
    @mlflow.trace(name="Orchestrator.generate_plan", span_type="AGENT")
    def generate_plan(self, *args, **kwargs):
        return super().generate_plan(*args, **kwargs)

    @mlflow.trace(name="Orchestrator.execute_plan", span_type="AGENT")
    def execute_plan(self, *args, **kwargs):
        return super().execute_plan(*args, **kwargs)

    @mlflow.trace(name="Orchestrator.run", span_type="AGENT")
...
def artifact_reference_uri(path_or_uri: str) -> str:
    """Return an MLflow dataset source URI for local paths and remote URIs."""
...
# Unlike the W&B run, this run stays open for the whole script: MLflow links
# traces to whichever run is active, so closing it here would detach them.
mlflow.start_run(run_name="spatial-ecological-my-dataset")

# MetaDataset records the dataset by reference (no upload), like
# wandb.Artifact.add_reference. Use mlflow.log_artifact to upload a copy instead.
dataset = MetaDataset(
    source=resolve_dataset_source(artifact_reference_uri(example_config.DATA_FILE)),
    name="input_dataset",
)
