...
STANDARD_DEFINITIONS: Dict[str, Dict[str, Dict[str, Any]]] = {
    ...
    # buckets so the field-driven planner (docs/development/plans/field-router.md)
...
# ShareTrait "basic" without any `trait_*` field. Those describe one row of a
# long-format trait table (`trait_name`, `trait_value`, `trait_unit`) or a property of
# that one measurement (`trait_type`, `trait_error_*`, `trait_converted`), so on wide
# data their answer is "every measurement column" — a column role, not a meaning any
# retrieval can match. Derived rather than copied so the two stay in step.
