...
1. Honor `tool_execution_mode: "llm"` in `Player` so parameterized tools become
   callable (unblocks 6 tools, one role).
2. Extend `_get_tool_context_compatibility()` with `TEXT` and `SQLITE` entries
   (unblocks both context types end-to-end).
3. Unrelated crashes found while reading: `PLANNING_TEMPERATURE` is used but not
   imported in `orchestrator/orchestrator.py:43`; `main.py:138-140` passes
   `args.model_name` / `args.temperature` / `args.provider`, none of which the
   parser defines. The library entry point works; `python -m src.main` does not.

## Not done

- No code changed. No tests added. Findings 1 and 2 are unfixed.
